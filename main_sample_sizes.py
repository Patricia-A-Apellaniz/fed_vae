...
def experiment(data_per_node, iid, n_samples_val, n_seeds_train, n_seeds_val, n_rounds, n_epochs_per_round,
               n_samples_public, m, l, dataset_name, max_samples_train, results_folder, n_jobs=10):
    ...
    os.makedirs(os.path.join(results_folder, dir_name), exist_ok=True)
    ...
    for exp_type in ['alone', 'favg', 'drs']:
        ...
        for i in range(n_nodes):
            ...
            df_val_list[i][0: m + 2 * l].to_csv(
                os.path.join(results_folder, dir_name, 'val_data_' + nodes[i].node_name + '.csv'), index=False)
        ...
        for round in range(nr):
            for node in nodes:
                ...
                node.public_df[0: m + 2 * l].to_csv(os.path.join(results_folder, dir_name,
        ...
        for i, node in enumerate(nodes):
            for j, model in enumerate(node.models):
                model.save(
                    os.path.join(results_folder, dir_name, 'model_' + exp_type + '_n_' + str(i) + '_m_' + str(j)) + '.pt')
        with open(os.path.join(results_folder, dir_name, 'training_' + exp_type + '.pkl'), 'wb') as f:
        ...
        _ = Parallel(n_jobs=n_jobs, verbose=10)(
            delayed(evaluate_node)(dataset_name, dir_name, exp_type, node_name, round, m, l,results_folder=results_folder, clas=clas, label=label,
                                   seed=seed) for node_name in node_names for seed in range(n_seeds_val) for round in
            range(nr))


def plot_experiment(dir_name, n_nodes, n_rounds, n_seeds_val, results_folder):
    def get_data(exp_type):
        with open(os.path.join(results_folder, dir_name, 'training_' + exp_type + '.pkl'), 'rb') as f:
            data = pickle.load(f)
        if exp_type == 'alone':
            dfs = [[pd.read_csv(os.path.join(results_folder, dir_name,
...
if __name__ == '__main__':
    ...
    # data_per_node_list = [(5, 500, 1000), (5, 10, 1000), (10, 100, 1000), (50, 100, 500)] # Extremo desbalance (mínimo y máximo distribuidos), fuerte desbalance (un nodo domina completamente), moderado desbalance (uno pequeño, otro mediano, otro grande), leve desbalance (progresivamente más muestras en cada nodo)
    data_per_node_list = [(50, 500, 1000), (50, 100, 1000), (100, 500, 1000), (50, 100, 500)]
    m = 7500
    l = 1000
    n_samples_val = m + 2 * l
    n_seeds_train = 3
    ...
    for dataset_name in dataset_names:
        for data_per_node in data_per_node_list:
            print('\n\nRunning experiment with data_per_node:', data_per_node)
            n_nodes = len(data_per_node)
            ...
            results_folder = 'results_' + str(data_per_node[0]) + '_' + str(data_per_node[1]) + '_' + str(data_per_node[2])
            if train_flag:
                # Using iid data
                experiment(data_per_node=data_per_node, iid=True, n_samples_val=n_samples_val, n_seeds_train=n_seeds_train,
                           ...
                           max_samples_train=max_samples_train, results_folder=results_folder)
            ...
            # Do the plots
            plot_experiment(dataset_name + '_iid', n_nodes, n_rounds, n_seeds_val, results_folder)
            plot_experiment(dataset_name + '_niid', n_nodes, n_rounds, n_seeds_val, results_folder)
